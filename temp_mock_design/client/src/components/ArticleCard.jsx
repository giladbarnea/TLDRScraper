...
// import './ArticleCard.css' - Removed custom CSS in favor of Tailwind
...
function ArticleCard({ article, index }) {
    ...
    rounded-2xl shadow-soft hover:shadow-soft-hover mb-4 overflow-hidden
    ${tldr.expanded ? "ring-1 ring-brand-100 shadow-soft-hover" : ""}
    ${isRemoved ? "opacity-60 bg-slate-50 border-dashed" : "hover:-translate-y-0.5"}
    ${isRead && !isRemoved ? "bg-slate-50/40" : ""}
  `;

	const titleClasses = `
    text-[18px] font-display font-bold leading-snug transition-colors duration-200
    ${isRemoved ? "text-slate-400 line-through" : "text-slate-900 group-hover:text-brand-600 cursor-pointer"}
    ${isRead && !isRemoved ? "text-slate-600 font-medium" : ""}
  `;
					...
					>
						{tldr.loading ? (
							<span className="flex items-center gap-1.5">
								<i className="w-3.5 h-3.5 animate-spin" data-lucide="loader-2"></i> Generating...
							</span>
						) : tldr.expanded ? (
							<span className="flex items-center gap-1.5">
								<i className="w-3.5 h-3.5" data-lucide="minimize-2"></i> Minimize
							</span>
						) : (
							<span className="flex items-center gap-1.5">
								<i className="w-3.5 h-3.5" data-lucide="sparkles"></i> TLDR
							</span>
						)}
					</button>
					...
					<div className="animate-slide-up mt-5 pt-5 border-t border-slate-100/80">
						<div className="flex items-center gap-2 mb-3">
							<div className="bg-brand-100 p-1 rounded-md">
								<i className="w-3.5 h-3.5 text-brand-600" data-lucide="bot"></i>
							</div>
