...
- **localStorage persistence**: "Select" button now saves component IDs to `localStorage` under key `'podcastSources-1'` with format `calendar-{date}` and `newsletter-{date}-{title}` (commit `51480fc`)
