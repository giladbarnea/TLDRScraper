...
Summary of plan: force the client to hit the server whenever the requested range includes today; the server would pull today's cached URLs, add them to `excluded_urls`, skip refetching those URLs, and return only new articles for the client to merge into Supabase.
...
Recommendation: Reject as-is. Update the plan to keep cached articles/issues in the response (either union cached+new on the server and recompute stats, or make `mergeWithCache` union cached payloads and recalc stats) and to preserve cache-first for non-today dates while still forcing a fresh scrape for today. Normalize "today" checks to date-only values before bypassing the cache.
