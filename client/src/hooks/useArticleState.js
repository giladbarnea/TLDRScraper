...
export function useArticleState(date, url) {
	...
	const state = !article ? 0 : article.removed ? 3 : article.tldrHidden ? 2 : article.read?.isRead ? 1 : 0;
	...
	const updateArticle = useCallback(
		(updater) => {
			...
			setPayload((current) => {
				...
				return {
					...current,
					articles: current.articles.map((a) => (a.url === url ? { ...a, ...updater(a) } : a)),
				};
			});
		},
	...
	const setTldrHidden = useCallback(
		(hidden) => {
