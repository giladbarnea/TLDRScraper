export async function scrapeNewsletters(startDate, endDate, cacheEnabled) {
  try {
    if (cacheEnabled && isRangeCached(startDate, endDate)) {
  ...
  return payloads.map(payload => {
    ...
    if (raw) {
      try {
        ...
        console.error(`Failed to merge with cache for ${key}:`, err)
      }
    }
