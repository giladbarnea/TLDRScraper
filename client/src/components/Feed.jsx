...
function DailyGroup({ payload }) {
  ...
  return (
    <section className="animate-slide-up">
      <FoldableContainer
        id={date}
        headerClassName="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-sm py-4 mb-6 border-b border-slate-200/60"
        title={
          <>
      ...
      >
        <div className="space-y-12">
          {issues.map((issue) => {
            ...
            const issueId = `${date}::${issue.category}`
          ...
          {articles.some((article) => !article.category) && (
            <FoldableContainer
              id={`${date}::Other`}
              className="space-y-6"
              headerClassName="pl-4 border-l-2 border-slate-200"
              title={
