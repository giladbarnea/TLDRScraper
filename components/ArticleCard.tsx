  ...
  return (
    <article className="group">
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="text-xs text-zen-subtle font-light">
          {article.source}
        </div>
        <button
      ...
      <div
        className={isExpanded ? 'mt-8 max-h-[500px] opacity-100 overflow-hidden transition-all duration-700 ease-in-out' : 'mt-0 max-h-0 opacity-0 overflow-hidden transition-all duration-700 ease-in-out'}
      >
        <div className="pt-6 border-t border-zen-line">
          {aiState === AIState.LOADING ? (
            ...
            <div className="text-xs font-light text-zen-subtle">
              {summaryText}
            </div>
          ) : (
